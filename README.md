<h3 align='center' size='big'>¡Hola! Soy mAngo 👋</h3>
<div align='center'>
  <img src='https://github.com/user-attachments/assets/d2de7ac8-254c-4a52-aead-ae8c10810f32' style='border-radius:50%;' width='200' />
</div>

<br>

<p align='center'>Desarrollo herramientas enfocadas en resolver problemas reales (y a veces por gusto),<br>con foco en rendimiento, usabilidad y una buena experiencia de desarrollo.</p>

<div align='center'>
  <a href='https://enlace'><img src='https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white' /></a>
  <a href='mailto:mangufaf@gmail.com'><img src='https://img.shields.io/badge/Correo-E41833?style=for-the-badge&logo=gmail&logoColor=white' /></a>
</div>

<br>

<p align='center'>
  <img src='https://skillicons.dev/icons?i=astro,preact,tailwind,ts,bun,express,sqlite,cloudflare' />
</p>

<h3>Mis proyectos principales</h3>

<table width="100%" align="center">
  <tr>
    <td width="50%" align="center">
      <a href="https://github.com/Ubiufboeuf/zenith">
        <img src="https://github.com/user-attachments/assets/a9911a3e-e95c-400a-a419-3a0789d77646" alt="Zenith - Sistema gestión y ventas" width="100%" />
        <br />
        <b>Zenith</b>
      </a>
    </td>
    <td width="50%" align="center">
      <a href="https://github.com/Ubiufboeuf/monado">
        <img src="https://github.com/user-attachments/assets/d47539c1-075e-47a3-a4f9-9e3ec6b28847" alt="Monado - Video Player" width="100%" />
        <br />
        <b>Monado</b>
      </a>
    </td>
  </tr>
  <tr>
    <td colspan='2' align="center">
    <br>
      <a href="https://github.com/Ubiufboeuf/yt-kit">
        <img src="https://github.com/user-attachments/assets/37c3cbcf-f888-4c3f-a421-7ac265adcc05" alt="@yt-kit/core - SDK" width="80%" />
        <br />
        <b>@yt-kit/core</b>
      </a>
    </td>
  </tr>
</table>

---

### Proyectos

- **Zenith**: Sistema POS enfocado en velocidad, usabilidad y flujo de ventas eficiente.
- **Monado**: Reproductor de video basado en YouTube donde el control lo tiene el usuario, no la plataforma.
- **yt-kit**: SDK modular para manejo de video, sin dependencias y optimizado.
